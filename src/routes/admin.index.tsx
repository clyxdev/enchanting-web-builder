...
import { LogOut, Plus, Pencil, Trash2, Server, ShieldCheck, Activity, Loader2, Power, Clock3, History } from "lucide-react";
import { AnimatePresence, motion } from "framer-motion";
import { toast } from "sonner";
import { z } from "zod";
import { supabase } from "@/integrations/supabase/client";
import { adminLogout, deleteAdminConfig, getAdminDashboard, saveAdminConfig } from "@/server/admin.functions";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function AdminDash() {
  ...
  const loadDashboard = useCallback(async (activeToken: string, quiet = false) => {
    try {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Dashboard</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage configs, track edits, and update the public site instantly.</p>
        </div>
      ...
      <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.06 } } }} className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <Stat icon={Server} label="Total Configs" value={stats.total} accent="text-cyan-600 bg-cyan-500/10" />
        <Stat icon={Activity} label="Active" value={stats.active} accent="text-emerald-600 bg-emerald-500/10" />
        <Stat icon={ShieldCheck} label="Protocols" value={stats.protocols} accent="text-violet-600 bg-violet-500/10" />
      </motion.div>
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {items.map((c) => (
                ...
                >
                  ...
                  <div className="md:col-span-3 truncate text-muted-foreground text-xs">{c.server}:{c.port}</div>
                  <div className="md:col-span-1">
                    <button
                      onClick={() => toggleActive(c)}
                      className={`text-[10px] font-medium px-2 py-1 rounded-md inline-flex items-center gap-1 ${
                        c.is_active ? "text-emerald-600 bg-emerald-500/10" : "text-muted-foreground bg-foreground/5"
                      }`}
                      title="Toggle active"
                    >
                  ...
                  <div className="md:col-span-2 flex md:justify-end gap-1">
                    ...
                    <button onClick={() => remove(c)} className="p-2 rounded-lg hover:bg-rose-500/10 text-muted-foreground hover:text-rose-500" aria-label="Delete">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="glass max-w-xl max-h-[88vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Protocol">
              <select className={inputCls} value={form.protocol} onChange={(e) => setForm({ ...form, protocol: e.target.value as Protocol })}>
                {(["VLESS", "VMESS", "TROJAN", "SSH"] as Protocol[]).map((p) => <option key={p} value={p} className="bg-background">{p}</option>)}
              </select>
            </Field>
            <Field label="Country">
            ...
            <Field label="Port">
              <input type="number" className={inputCls} value={form.port} onChange={(e) => setForm({ ...form, port: parseInt(e.target.value || "0", 10) })} />
            </Field>
            <Field label="Notes" full>
              <input className={inputCls} value={form.notes ?? ""} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
            </Field>
            <Field label="Config String" full>
              <textarea rows={4} className={`${inputCls} font-mono text-xs`} value={form.config_string} onChange={(e) => setForm({ ...form, config_string: e.target.value })} />
            </Field>
            <Field label="" full>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <button onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg text-sm hover:bg-foreground/5">Cancel</button>
            <button onClick={save} disabled={saving} className="px-4 py-2 rounded-lg bg-foreground text-background text-sm font-medium inline-flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-60">
...
const inputCls = "w-full glass rounded-xl px-3 py-2 text-sm outline-none focus:border-foreground/30 transition-colors bg-background/40";
...
function Stat({ icon: Icon, label, value, accent }: { icon: React.ComponentType<{ className?: string }>; label: string; value: number; accent: string }) {
  return (
    <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }} className="glass rounded-2xl p-5 flex items-center gap-4">
...
function AuditPanel({ logs }: { logs: AuditLog[] }) {
  return (
    <motion.aside initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-5 h-fit">
      ...
      {logs.length === 0 ? (
        ...
        <div className="space-y-3">
          <AnimatePresence initial={false}>
            {logs.map((log) => (
              <motion.div key={log.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} className="rounded-xl border border-foreground/5 bg-foreground/[0.025] p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-medium capitalize">{log.action} · {log.config_name}</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">by {log.actor_name}{log.protocol ? ` · ${log.protocol}` : ""}</p>
                  </div>
                  <Clock3 className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
                </div>
                <p className="text-[11px] text-muted-foreground mt-2">{new Date(log.created_at).toLocaleString()}</p>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
